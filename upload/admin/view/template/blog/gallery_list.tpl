...
function filter() {
	url = 'index.php?route=blog/gallery&token=<?php echo $token; ?>';
	...
	<!-- ocshop -->
	var filter_news_id = $('select[name=\'filter_news_id\']').attr('value');
							 
	if (filter_news_id) {
		url += '&filter_news_id=' + encodeURIComponent(filter_news_id);
	}
	<!-- ocshop -->
...
$('input[name=\'filter_name\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=blog/gallery/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
			dataType: 'json',
...
$('input[name=\'filter_model\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=blog/gallery/autocomplete&token=<?php echo $token; ?>&filter_model=' +  encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {		
				response($.map(json, function(item) {
					return {
...
<script type="text/javascript"><!--
  $(document).ready(function() {
        $('input[name=\'status\']').change(function() {
         $.post('index.php?route=blog/article/status&token=<?php echo $token; ?>', 'status=' + ($(this).attr('checked') ? '1' : '0') + '&article_id=' + $(this).val());
